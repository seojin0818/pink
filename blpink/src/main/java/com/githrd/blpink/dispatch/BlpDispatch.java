...
import com.githrd.blpink.dispatch.*;

/**
 * 
 * 이 클래스는 .black이라는 확장자로 요청이 된 경우
 * 요청 내용을 분석하고 실행할 클래스를 선택해서 실행(디스패치)시켜줄 서블릿클래스
 * @author 백서진
 * @since 2022/05/11
 * @version v.1.0
 * @see
 * 			com.githrd.blpink.controller.BlpInter
 * 			com.githrd.blpink.resources.blp.properties
 * 
 * 			작업이력 ]
 * 
 * 				2022.05.11	-	클래스 제작
...
public class BlpDispatch extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			path = path + "../resources/bl.properties";
//			System.out.println("### dispatch path : " + path);
	...
	/*
		...
		.black이라는 확장자로 요청이 왔을 때 매번 실행되는 함수
